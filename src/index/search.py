...
class Search():
    ...
    def lookup(self, queries):
        ...
        for idx in range(len(queries)):
            i = query_object.find_k_nearest_neighbors(queries[idx], 5) # search top-5 matches for each subfingerprint
            cands += len(query_object.get_unique_candidates(queries[idx]))
            emb_idx.append(i)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        s = time.time()
        songid, timeoffset, levi,cands  = API.subfingerprints_search(clean_query)
